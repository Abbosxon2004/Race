...
//        String race_name = (String) req.getAttribute("race_name");
        ...
        if (race_name!=null) {
            ...
            success = dao.bettingMoney(username, race_name, for_betting_money, horse_name, horse_position);
        } else success = false;
        String message = null;
        if (success)
...
//        req.getRequestDispatcher("/getProfileDetails").forward(req, resp);
        resp.sendRedirect("http://localhost:8080/Race_war_exploded/getProfileDetails?message="+message);
    }
}
